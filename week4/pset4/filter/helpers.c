#include "helpers.h"
#include <math.h>
void avgred(int i, int j, int height, int width, RGBTRIPLE image[height][width], float *red, float *blue, float *green);
...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            float blue;
            float green;
            avgred(i, j, height, width, tmp, &red, &blue, &green);
...
void avgred(int i, int j, int height, int width, RGBTRIPLE image[height][width], float *red, float *blue, float *green)
{
    float sum1 = 0, sum2 = 0, sum3 = 0, avg;
    ...
    for (int y = k; y <= l; y++)
    {
        for (int z = m; z <= n; z++)
        {
            sum1 += image[y][z].rgbtRed;
            sum2 += image[y][z].rgbtBlue;
            sum3 += image[y][z].rgbtGreen;
            counter++;
        }
    }
    *red = sum1 / counter;
